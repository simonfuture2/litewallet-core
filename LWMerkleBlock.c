...
{
    LWMerkleBlock *block = calloc(1, sizeof(*block));
...
{
    LWMerkleBlock *cpy = LWMerkleBlockNew();

    assert(block != NULL);
    *cpy = *block;
    cpy->hashes = NULL;
    cpy->flags = NULL;
    LWMerkleBlockSetTxHashes(cpy, block->hashes, block->hashesCount, block->flags, block->flagsLen);
    return cpy;
}
...
{
    LWMerkleBlock *block = (buf && 80 <= bufLen) ? LWMerkleBlockNew() : NULL;
    size_t off = 0, len = 0;
    ...
    if (block) {
        ...
        if (off + sizeof(uint32_t) <= bufLen) {
            ...
            off += sizeof(uint32_t);
            block->hashesCount = (size_t)LWVarInt(&buf[off], (off <= bufLen ? bufLen - off : 0), &len);
            off += len;
            ...
            off += len;
            block->flagsLen = (size_t)LWVarInt(&buf[off], (off <= bufLen ? bufLen - off : 0), &len);
            off += len;
            len = block->flagsLen;
        ...
        LWSHA256_2(&block->blockHash, buf, 80);
        LWScrypt(&block->powHash, sizeof(block->powHash), buf, 80, buf, 80, 1024, 1, 1);
    }
...
size_t LWMerkleBlockSerialize(const LWMerkleBlock *block, uint8_t *buf, size_t bufLen)
{
    ...
    if (block->totalTx > 0) {
        len += sizeof(uint32_t) + LWVarIntSize(block->hashesCount) + block->hashesCount*sizeof(UInt256) +
               LWVarIntSize(block->flagsLen) + block->flagsLen;
    }
    ...
    if (buf && len <= bufLen) {
        ...
        if (block->totalTx > 0) {
            ...
            off += sizeof(uint32_t);
            off += LWVarIntSet(&buf[off], (off <= bufLen ? bufLen - off : 0), block->hashesCount);
            if (block->hashes) memcpy(&buf[off], block->hashes, block->hashesCount*sizeof(UInt256));
            off += block->hashesCount*sizeof(UInt256);
            off += LWVarIntSet(&buf[off], (off <= bufLen ? bufLen - off : 0), block->flagsLen);
...
static size_t _LWMerkleBlockTxHashesR(const LWMerkleBlock *block, UInt256 *txHashes, size_t hashesCount, size_t *idx,
                                      size_t *hashIdx, size_t *flagIdx, int depth)
{
    ...
    if (*flagIdx/8 < block->flagsLen && *hashIdx < block->hashesCount) {
        ...
        if (! flag || depth == _ceil_log2(block->totalTx)) {
            ...
            _LWMerkleBlockTxHashesR(block, txHashes, hashesCount, idx, hashIdx, flagIdx, depth + 1); // left branch
            _LWMerkleBlockTxHashesR(block, txHashes, hashesCount, idx, hashIdx, flagIdx, depth + 1); // right branch
        }
    }
...
size_t LWMerkleBlockTxHashes(const LWMerkleBlock *block, UInt256 *txHashes, size_t hashesCount)
{
    ...
    return _LWMerkleBlockTxHashesR(block, txHashes, (txHashes) ? hashesCount : SIZE_MAX, &idx, &hashIdx, &flagIdx, 0);
}
...
// NOTE: this merkle tree design has a security vulnerability (CVE-2012-2459), which can be defended against by
// considering the merkle root invalid if there are duplicate hashes in any rows with an even number of elements
static UInt256 _LWMerkleBlockRootR(const LWMerkleBlock *block, size_t *hashIdx, size_t *flagIdx, int depth)
{
    uint8_t flag;
    ...
    if (*flagIdx/8 < block->flagsLen && *hashIdx < block->hashesCount) {
        ...
        if (flag && depth != _ceil_log2(block->totalTx)) {
            hashes[0] = _LWMerkleBlockRootR(block, hashIdx, flagIdx, depth + 1); // left branch
            hashes[1] = _LWMerkleBlockRootR(block, hashIdx, flagIdx, depth + 1); // right branch
            ...
            if (! UInt256IsZero(hashes[0]) && ! UInt256Eq(hashes[0], hashes[1])) {
                ...
                LWSHA256_2(&md, hashes, sizeof(hashes));
            }
...
int LWMerkleBlockIsValid(const LWMerkleBlock *block, uint32_t currentTime)
{
    ...
    UInt256 merkleRoot = _LWMerkleBlockRootR(block, &hashIdx, &flagIdx, 0), t = UINT256_ZERO;
    int r = 1;
...
// frees memory allocated by LWMerkleBlockParse
